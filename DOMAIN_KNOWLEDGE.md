# DOMAIN\_KNOWLEDGE.md — Cómo funciona el negocio (no cómo está hecho el software)

**Fecha:** 2026-07-11 **Estado:** Canonical — lectura OBLIGATORIA antes de construir cualquier feature de pricing/revenue **Por qué existe:** los 60 documentos describen la ARQUITECTURA del software. Ninguno describe CÓMO FUNCIONA EL NEGOCIO. Sin esto, el sistema construye software correcto que resuelve el problema equivocado. Este documento es el conocimiento de dominio que el propietario transfirió y que el corpus nunca capturó.
...
## 1.3 Mínimos de estadía (min\_nights)
...
3. **TECHO DE PRECIO POR DESTINO ALTERNATIVO (concepto central, no está en ningún manual):** El precio máximo de una unidad en Costa Atlántica NO lo define la competencia local. Lo define el **costo de un viaje alternativo comparable** — típicamente **Brasil**.  
...
1. **Resolver la temporada correcta** (paquete vs. noche, L-J vs V-D, min\_nights)  
...
Propietario: "precio para 4 personas en enero" Sistema: "/pricing requiere: unidad, fecha inicio, fecha fin"

**BIEN (lo que debe ser):**

Propietario: "precio para 4 personas en enero" Sistema: "Para enero (temporada alta, paquete semanal, mínimo 7 noches), tenés 2 unidades para 4 personas: • UF214 (cap 4): semana $700. Competencia comparable: LINDA STUDIO 60 (link) a $750 — está 7% arriba, pero es 5m² más grande, así que en términos ajustados estamos parejos. • UF508 (cap 6): semana $1000. LINDA 205 DUPLEX (link) a $1100. Tu ocupación de enero está en 45% a 5 meses vista; el año pasado a esta altura tenías 70%. **Vas atrasado.** Sugerencia: mantener UF214 y bajar UF508 a $950 para acelerar. ¿Querés que analicemos alternativas?"

Propietario: "¿y si dejo el duplex en 1000?" Sistema: \[RECUERDA el contexto anterior\] "Si mantenés $1000 con el pace actual, proyectando..., podrías terminar enero con \~65% de ocupación en vez de \~85%. Revenue estimado: $X vs $Y. La diferencia es \~$Z."
...
**Nota de orden:** esta Parte 5 (umbrales operativos) se lee junto con la Parte 6 (normalización monetaria y anticipación), que la complementa. Ambas forman el núcleo de revenue.
...
| :---- | :---- | :---- |
| **Baja/media** (per\_night) | **1 noche suelta** ya es un gap | Ofrecer a huéspedes de la reserva anterior o posterior (extender su estadía). Es la vía más barata: ya están ahí, cero costo de adquisición |
...
pace\_actual \= noches vendidas para \[período target\] al día de hoy

pace\_referencia \= noches vendidas para \[mismo período\] a la misma distancia temporal, en años anteriores

                  (de pricing\_history \+ reservations históricas)
...
# PARTE 6 — NORMALIZACIÓN MONETARIA Y ANTICIPACIÓN DE MERCADO (la tesis de revenue)

**Por qué esta parte es la más importante:** define la diferencia entre un sistema que COPIA la competencia y uno que ANTICIPA el mercado. Lo segundo es el moat real (ver ATLAS\_MOAT\_AGENTICO\_TESIS.md).

## 6.1 El problema de comparar precios históricos en Argentina

Un precio de enero 2024 NO es comparable directamente con enero 2026\. Entre medio hubo inflación en pesos, movimiento del dólar, y cambios de poder adquisitivo. Leer el número histórico crudo lleva a decisiones erróneas.

**Para aprender del histórico, hay que NORMALIZARLO:**

- USD es la moneda canónica (ya definido), pero incluso el USD tuvo poder de compra distinto por ciclo  
- Al analizar un enero pasado, traducirlo a términos comparables con hoy: ¿qué representaba ese precio en relación al dólar real y al poder de compra de ese momento?  
- El sistema debe guardar, junto a cada precio histórico, el CONTEXTO de ese momento (tipo de cambio, y si es posible un índice de referencia) para poder normalizar

## 6.2 Inflación vs. dólar: qué manda al actualizar tarifas

**No es "actualizar por inflación" O "por dólar". Es entender qué restricción manda.**

Caso difícil (real): inflación 30% en pesos, dólar sube solo 5%.
...
## 6.3 Anticipación de mercado — ser el primero (LA TESIS)

**El insight central del propietario:** cuando nadie publicó tarifas para una fecha lejana, alguien es el primero. Si ATLAS puede ser ese primero —con criterio, no al azar— anticipa el mercado en vez de copiarlo.

**Por qué esto es el moat:**

- Un revenue manager que solo copia competencia SIEMPRE va un paso atrás (reacciona a lo que otros ya decidieron)  
- ATLAS, con histórico normalizado \+ estacionalidad \+ contexto macro, puede PROPONER la tarifa de enero en julio, cuando nadie más publicó  
- Anticipar bien \= capturar reservas tempranas (early bookers) a buen precio, antes que la competencia reaccione  
- Esto genera MÁS reservas anticipadas, no solo mejores precios

**Cómo el sistema decide el precio de una fecha lejana (sin competencia disponible):**

1. Histórico propio normalizado de esa temporada (§7.1)  
2. Estacionalidad (§1.2)  
3. Contexto macro actual (ciclo dólar/inflación, §7.2)  
4. Posicionamiento entre unidades propias (ratio histórico)  
5. Techo estimado por destino alternativo (aunque la competencia local no haya publicado, el costo de Brasil sí es estimable)  
6. **Propone con CONFIANZA CALIBRADA** — declara que es anticipación, no seguimiento

**El riesgo de ser primero (y cómo mitigarlo):**

- Ser el primero \= ser el que se equivoca solo si falla. Muy alto → perdés early bookers. Muy bajo → dejás plata  
- Mitigación: APRENDIZAJE. ATLAS propone, mide el resultado (¿se llenó al ritmo esperado?), y ajusta. Cada temporada mejora su anticipación  
- La confianza de una recomendación anticipada debe ser explícita: "esta es una tarifa de anticipación basada en tu histórico; el mercado aún no publicó. La ajustaré cuando aparezca la competencia"

## 6.4 Fuente de verdad según anticipación (regla operativa)

| Anticipación | Fuente primaria | Competencia | Declara |
| :---- | :---- | :---- | :---- |
| **Lejana** (meses, nadie publicó) | Histórico normalizado \+ estacionalidad \+ macro | No disponible | "anticipación basada en tu histórico; competencia no publicada aún" |
| **Media** (algunos publicaron) | Histórico \+ competencia parcial | Parcial, ponderada | "competencia parcial disponible" |
| **Cercana** (mercado publicado) | Competencia real \+ techo real \+ pace | Completa | "basado en mercado actual" |

**Crítico:** NUNCA mostrar un techo o competencia de EJEMPLO como si fuera real. Si el dato no está publicado aún, decirlo — un número inventado destruye la confianza del propietario más que admitir "esto todavía no lo sé".

## 6.5 Aprendizaje continuo (cómo ATLAS mejora su anticipación)

Cada ciclo alimenta el siguiente:

1. ATLAS propone tarifa anticipada para \[temporada\]  
2. Se registra la propuesta \+ el contexto (fecha, macro, confianza)  
3. A medida que llegan reservas, se mide: ¿el pace fue el esperado? ¿el precio fue aceptado?  
4. Cuando la competencia publica, se compara: ¿ATLAS anticipó bien vs. el mercado que llegó después?  
5. Ese aprendizaje ajusta la próxima anticipación

Esto es el Decision Graph (ARCH-0013) aplicado a revenue: el histórico de decisiones \+ resultados se vuelve el activo que ningún competidor nuevo tiene. **Es el moat que se construye con tiempo y uso, no con features.**

---

# PARTE 7 — MEJORAS FUTURAS (Fase 3+, registradas, NO construir ahora)
