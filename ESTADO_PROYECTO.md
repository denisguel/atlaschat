...
| Resiliencia proveedor gratuito (Telegram): retry cubre 429/over-capacity/timeout+red; fallback Groq→Gemini (gemini-flash-latest, el que tiene quota free); ProviderSaturatedError → mensaje claro al usuario (nunca mudo, incl. webhook fuera de orchestrate) | HECHO | 2026-07-18 | tsc=0, 114/114; e2e: Groq falla→Gemini responde, ambos caídos→mensaje de saturación (no silencio) |
