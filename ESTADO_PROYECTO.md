...
| PXSOL (`WEB_DIRECT_API`): DIAGNOSTICADO con el dato crudo del endpoint. El init de dos pasos (POST `search/insert` → 302 con `SearchID` en el Location → `list6`) **FUNCIONA por búsqueda**: devuelve un SearchID fresco por fecha, y el adapter ya usa ese flujo (`config.init` presente), NO el SearchID fijo. El SearchID fijo del `_fallback_sin_init` (66187729) está MUERTO (ProductList len 0). El 0/121 `had_data=false` es LEGÍTIMO: PXSOL no tiene NINGUNA tarifa publicada en todo el horizonte hoy (todo `PromotionalRate=99999999999`/Availability=0 o ProductList vacío) — carga discontinua real, no bug del adapter (verificado end-to-end: `webDirectApiAdapter.fetch` → `ok:true prices=0`) | DIAGNOSTICADO | 2026-07-21 | init dinámico confirmado por búsqueda (SearchID fresco cada corrida). Dos LANDMINES latentes flagueadas: (a) `sku_mapping` DESACTUALIZADO — el config mapea `14018→UF508` pero la API hoy devuelve SKUs `14014,14017,14020`; `14014→UF214` sigue válido, pero cuando el duplex vuelva a publicar su tarifa caería en un SKU sin mapear → se perdería (mismo patrón que la capacidad null de Booking). No se puede confirmar el SKU correcto sin una tarifa viva que exponga `SkuTitle` (hoy viene vacío). (b) el `_fallback_sin_init` del config tiene una nota engañosa ("es lo que corre hoy y devuelve precios reales" — falso). Ninguna se toca sin dato vivo (§3.2: no inventar el mapping) |
...
| Alarma de FEED MUERTO del techo (observabilidad): una fecha vacía es carga discontinua normal, pero si la fuente `is_ceiling` devuelve `had_data:false` en MASA en una corrida (muchas fechas intentadas, CERO con datos) es un feed caído que degradaría en silencio todas las recomendaciones (el techo deja de capar sin aviso) — la clase de bug que ya pasó (PXSOL 0/121 estuvo semanas sin saltar). `ceilingFeedIsSilent()` (puro): con muestra suficiente (default ≥10) y cero datos → alarma. El cron acumula salud por (propiedad, fuente is_ceiling) contando SOLO intentos `ok` (una caída de red no es feed vacío) y loguea a `execution_logs` (BDVL, RIESGO ALTO) + `console.error` | HECHO | 2026-07-21 | commit 8b0639f; verificado con dato REAL (PXSOL vacío en todo el horizonte): cron `--dates` sobre 10 fechas → alarma disparada, fila en `execution_logs`: `scraper:ceiling_feed_dead:Reserva Directo (complejo LindaBay):0/10_con_datos:high_risk`. tsc=0, 143/143 (4 tests del umbral) |
